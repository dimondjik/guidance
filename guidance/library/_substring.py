...
class SuffixAutomaton:
    ...
    def __init__(self, s):
        ...
        self.MAXLEN = 2 * len(
            s
        )  # we can't have more than 2n-1 states for a string of length n
...
def substring(lm, target_string: str, name: Optional[str] = None):
    ...
    while state_stack:
        ...
        if not state.next:
            node_cache[state_ind] = string(
                ""
            )  # Leaf nodes represent empty string suffixes
            state_stack.pop()
            continue
        ...
        if unprocessed_children:
            ...
            node_cache[state_ind] = (
                optional(select(options, skip_checks=True))
                if len(options) > 1
                else optional(options[0])
            )
            state_stack.pop()
